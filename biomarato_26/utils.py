...
def load_maps():
    ...
    project_ids = {
        "Barcelona": config.PROJECTS_BY_NAME["Barcelona"],
        "Tarragona": config.PROJECTS_BY_NAME["Tarragona"],
        "Girona": config.PROJECTS_BY_NAME["Girona"],
        "Catalunya": config.PROJECTS_BY_NAME["Catalunya"],
    }
...
def get_metrics_province():
    # TODO: Datos falsos para pruebas - descomentar código original cuando haya datos reales
    fake_data_path = f"{directory}/data/province_metrics_fake.csv"
    if os.path.exists(fake_data_path):
        return pd.read_csv(fake_data_path)

    result = []
    with requests.Session() as session:
        for k, v in config.PROJECTS_BY_NAME.items():

            # ids distintos al proyecto principal
            if v != config.MAIN_PROJ:
...
def fig_provinces(main_metrics: pd.DataFrame, field: str, title: str):
    """
    Generate a treemap chart of the main metrics for each province.

    Parameters:
    - main_metrics (DataFrame): The main metrics data.
    - field (str): The field to use for the values.
    - title (str): The title of the chart.

    Returns:
    - fig (plotly.graph_objects.Figure): The generated treemap chart.
    - None if dataframe is empty or all values are zero.
    """
    if main_metrics is None or main_metrics.empty:
        return None
    # Check if the specific field has any non-zero values
    if field not in main_metrics.columns or main_metrics[field].sum() == 0:
        return None
    provincias = list(config.PROJECTS_BY_NAME.keys())
    color_map = {
        provincias[0]: config.COLORS[4],
        provincias[1]: config.COLORS[3],
        provincias[2]: config.COLORS[1],
    }
...
@st.cache_data(ttl=3600)
def get_grouped_monthly(project_id: int) -> pd.DataFrame:

    session = requests.Session()

    meses = {
        f"{config.YEAR}-05": ["01", "31"],
        f"{config.YEAR}-06": ["01", "30"],
        f"{config.YEAR}-07": ["01", "31"],
        f"{config.YEAR}-08": ["01", "31"],
        f"{config.YEAR}-09": ["01", "30"],
        f"{config.YEAR}-10": ["01", "15"],
    }
    ...
    for mes, limits in meses.items():
        ...
        url_obs = f"{config.API_PATH}/observations"
        url_spe = f"{config.API_PATH}/observations/species_counts"
        url_observers = f"{config.API_PATH}/observations/observers"
...
def fig_multi_year_comparison(df_list, years, field, colors):
    ...
    Returns None if all dataframes are empty.
    """
    if len(df_list) != len(years) or len(df_list) != len(colors):
        return None

    # Check if all dataframes are empty
    if all(df is None or df.empty for df in df_list):
        return None
