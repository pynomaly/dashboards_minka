...
def _fetch_daily_metrics_batch(session, proj_id, day_batches, urls):
    ...
    def fetch_single_day(day_str):
        ...
        try:
            ...
            with ThreadPoolExecutor(max_workers=3) as day_executor:
                metric_futures = {
                    day_executor.submit(session.get, url, params=params, timeout=10): metric
    ...
    with ThreadPoolExecutor(max_workers=8) as executor:
        day_futures = {executor.submit(fetch_single_day, day_str): day_str for day_str in day_batches}
...
def get_list_users(id_project):
    ...
    with ThreadPoolExecutor(max_workers=2) as executor:
        future_users = executor.submit(session.get, url1, timeout=15)
        future_identifiers = executor.submit(session.get, url2, timeout=15)
...
def get_new_data(project, grade=None):
    ...
    try:
        ...
        df_obs = pd.read_csv(obs_file, usecols=['id'] if os.path.exists(obs_file) else None)
        ...
        if len(obs) > 0:
            ...
            df_photos_updated = pd.concat([df_photos_full, df_photos2], ignore_index=True)
            
            # Save with compression to reduce I/O time
            df_obs_updated.to_csv(obs_file, index=False, compression='gzip' if obs_file.endswith('.gz') else None)
            df_photos_updated.to_csv(photos_file, index=False, compression='gzip' if photos_file.endswith('.gz') else None)
            ...
            for chunk_start in range(1, num_pages + 1, chunk_size):
                ...
                with ThreadPoolExecutor(max_workers=6) as executor:
                    ...
                    for page in range(chunk_start, chunk_end):
                        ...
                        chunk_futures[executor.submit(session.get, url, params=page_params, timeout=15)] = page
...
def _process_species_with_obs(species_df, proj_id, session, type="project"):
    ...
    chunk_size = 50
    species_chunks = [species_df[i:i + chunk_size] for i in range(0, len(species_df), chunk_size)]
    ...
    for chunk in species_chunks:
        ...
        with ThreadPoolExecutor(max_workers=8) as executor:
            futures = {
                executor.submit(get_first_obs_taxon, row["id"], proj_id, type, session): idx
    ...
    return result_df.sort_values(by=["first_date", "obs_id"], ascending=False, na_position='last').reset_index(drop=True)
...
if __name__ == "__main__":
    ...
    with ThreadPoolExecutor(max_workers=3) as executor:
        project_futures = [executor.submit(_process_project, proj_id) for proj_id in all_projects]
    ...
    with requests.Session() as session:
        ...
        try:
            ...
            downloaded_species_biomarato = pd.read_csv(f"{directory}/data/place_biomarato_species.csv")
            
            if species_biomarato is not None and len(species_biomarato) != len(downloaded_species_biomarato):
                species_biomarato = _process_species_with_obs(species_biomarato, place_biomarato, session, "place")
                species_biomarato.to_csv(f"{directory}/data/place_biomarato_species.csv", index=False)
