...
PROJECTS = {
    config.PROJ_NAME: config.MAIN_PROJ,
    "BioMARatona 2025": 424,
    "BioMARatona 2024": 452,
}
...
def load_map_data_optimized(proj_id):
    ...
    try:
        ...
        if len(df_map) > MAX_MAP_POINTS:
            ...
            st.info(
                f"Visualização de uma amostra de {MAX_MAP_POINTS:,} pontos para um melhor desempenho."
            )
        ...
        st.error(f"⚠️ Erro ao carregar os dados do mapa: {e}")
        return None
    ...
    if map_key not in st.session_state:
        try:
            with st.spinner(f"Creating maps for {project_name}..."):
                ...
                st.success(f"Mapas criados com {len(df_map):,} pontos de dados")

        except Exception as e:
            st.error(f"⚠️ Erro ao criar mapas para {project_name}: {e}")
...
if map_key in st.session_state and st.session_state[map_key] is not None:
    ...
    st.info("Não existem mapas disponíveis para exibição.")
