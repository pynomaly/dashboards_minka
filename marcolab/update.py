...
all_projects = [547, 580, 581]

# place to search first time taxon occurrence
GLOBAL_PLACE = 396
...
def _fetch_daily_metrics_batch(session, p_id, day_batches, urls, type="project"):
    """Fetch metrics for multiple days with parallel processing"""
    from concurrent.futures import ThreadPoolExecutor, as_completed

    headers = {"Authorization": f"Bearer {access_token}"}

    def fetch_day_metrics(day_str, type="project"):
    ...
    with ThreadPoolExecutor(max_workers=5) as executor:
        day_futures = {
            executor.submit(fetch_day_metrics, day_str, type=type): day_str
...
def update_main_metrics(proj_id: int, type="project") -> pd.DataFrame:
    """
    ...
    # Fecha de inicio de Pharos: 2024/09/01
    first_day = datetime.date(year=2024, month=9, day=1)
    today = datetime.datetime.today().date()
    rango_temporal = (today - first_day).days
    ...
    valid_results = _fetch_daily_metrics_batch(
        session, proj_id, valid_days, urls, type=type
    )
...
def get_list_users(p_id, type="project"):
    """Parallelized version using concurrent API calls"""

    session = _get_global_session()
    headers = {"Authorization": f"Bearer {access_token}"}

    url1 = f"{API_PATH}/observations/observers?{type}_id={p_id}&quality_grade=research"
    url2 = (
        f"{API_PATH}/observations/identifiers?{type}_id={p_id}&quality_grade=research"
    )
    ...
    try:
        ...
        print(f"Error fetching user data for project {p_id}: {e}")
        return pd.DataFrame(
...
def get_first_obs_taxon(taxon_id, place_id, session=None):
    if session is None:
        session = requests.Session()
...
def _process_project(p_id, grade=None, type="project"):
    """Process a single project with optimized operations"""
    print("Update df:", p_id)

    try:
        # Ya existe el archivo
        downloaded_obs = pd.read_csv(f"{directory}/data/{p_id}_df_obs.csv")
        if type == "project":
            obs = get_obs(id_project=p_id, grade=grade)
        elif type == "place":
            obs = get_obs(place_id=p_id, grade=grade)

        if len(obs) > 0 and len(obs) != len(downloaded_obs):
            df_obs, df_photos = get_dfs(obs)
            pt_users = get_list_users(p_id, type=type)

            # Save files
            df_obs.to_csv(f"{directory}/data/{p_id}_df_obs.csv", index=False)
            df_photos.to_csv(f"{directory}/data/{p_id}_df_photos.csv", index=False)
            pt_users.to_csv(f"{directory}/data/{p_id}_pt_users.csv", index=False)

            print(f"Updated files for project {p_id}")
            return True
...
def _process_species_with_obs(species_df, place_id, session):
    ...
    for idx, row in species_copy.iterrows():
        try:
            result = get_first_obs_taxon(row["id"], place_id, session)
...
def _process_species_with_obs_concurrent(species_df, place_id, session):
    ...
    def fetch_single_observation(idx_row_tuple):
        ...
        try:
            result = get_first_obs_taxon(row["taxon_id"], place_id, session)
            return idx, result
        except Exception as e:
            print(f"Error fetching observation for taxon at index {idx}: {e}")
            return idx, [None, None, None, None]

    species_copy = species_df.copy().reset_index(drop=True)

    # Initialize columns before concurrent processing
    species_copy["first_date"] = None
    species_copy["author"] = None
    species_copy["obs_id"] = None
    species_copy["photo_url"] = None
    ...
    with ThreadPoolExecutor(max_workers=5) as executor:
        ...
        for future in as_completed(future_to_idx):
            ...
            species_copy.at[idx, "first_date"] = result[0]
            species_copy.at[idx, "author"] = result[1]
            species_copy.at[idx, "obs_id"] = result[2]
            species_copy.at[idx, "photo_url"] = result[3]
...
def process_project_species(p_id):
    """Process species for a single project"""
    try:
        print(f"Get species for project {p_id}")
        species = get_list_species(p_id, type="project")
        try:
            downloaded_species = pd.read_csv(f"{directory}/data/{p_id}_species.csv")
        except FileNotFoundError:
        ...
        if species is not None and len(species) != len(downloaded_species):
            with requests.Session() as session:
                ...
                species = _process_species_with_obs_concurrent(species, p_id, session)
            species.to_csv(f"{directory}/data/{p_id}_species.csv", index=False)
            print(f"Species updated for {p_id}")
            return f"Success: {p_id}"
        return f"No update needed: {p_id}"
    except Exception as e:
        error_msg = f"Error processing species for project/place {p_id}: {e}"
        print(error_msg)
        return error_msg
...
if __name__ == "__main__":
    ...
    for p_id in all_projects:
        main_metrics_df = update_main_metrics(p_id)
        main_metrics_df.to_csv(f"{directory}/data/{p_id}_main_metrics.csv", index=False)
        print(f"Main metrics {p_id} actualizada")

    # Process projects sequentially
    for p_id in all_projects:
        _process_project(p_id)
    ...
    with ThreadPoolExecutor(max_workers=3) as executor:
        ...
        print("Get species general project")
        for p in all_projects:
            try:
                species = get_list_species(p)

                with requests.Session() as session:
                    session.headers.update({"Connection": "keep-alive"})
                    species = _process_species_with_obs_concurrent(
                        species, GLOBAL_PLACE, session
                    )
                species.to_csv(f"{directory}/data/{p}_species.csv", index=False)
                print(f"Species updated for {p}")
            except Exception as e:
