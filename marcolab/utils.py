...
projects = [
    {"id": 581, "name": "MarCoLab Lanzarote"},
    {"id": 580, "name": "MarCoLab Gran Canaria"},
]
...
def load_maps():
    ...
    with st.container():
        ...
        with col2:
            st.header(":blue[Mapes]")

    # Project selection
    project_name = st.selectbox(
        label="Proyecto para mostrar en el mapa",
        options=("MarCoLab", "MarCoLab Lanzarote", "MarCoLab Gran Canaria"),
...
def fig_area_evolution(df, field, title, color):
    ...
    """
    df[parser_lang["date"]] = pd.to_datetime(df[parser_lang["date"]])
    df = df[df[parser_lang["date"]] >= "2025-01-01"].copy()
...
def fig_bars_months(grouped: pd.DataFrame, field: str, title: str, color: str):
    ...
    fig.update_layout(
        ...
        xaxis=dict(
            tickmode="array",
            tickvals=grouped[parser_lang["date"]].to_list(),
            ticktext=grouped[parser_lang["date"]].to_list(),
            tickfont=dict(size=14),
            tickangle=-45,
        ),
    )
    fig.update_yaxes(range=[0, max_val * 1.20], automargin=True)
    return fig
...
@st.cache_resource(ttl=3600)
def create_markercluster(df, zoom=6):
    ...
    return _create_javascript_map(df_clean, center, zoom)


def _create_javascript_map(df, center, zoom=6):
...
def get_grouped_monthly(project_id: int) -> pd.DataFrame:
    ...
    # Generar lista de (año, mes) desde enero 2025 hasta el mes actual
    months = pd.period_range(start=start, end=today, freq="M")
    ...
    for p in months:
        ...
        # Si es el mes actual, no pasar de hoy
        ...
        params = {
            ...
            "d1": first_day.isoformat(),
            "d2": last_day.isoformat(),
        }

        month_data = {
            parser_lang["date"]: first_day.strftime("%Y-%m"),
            parser_lang["observations"]: session.get(url_obs, params=params).json()[
                "total_results"
            ],
            parser_lang["species"]: session.get(url_spe, params=params).json()[
                "total_results"
            ],
            parser_lang["participants"]: session.get(
                url_observers, params=params
            ).json()["total_results"],
        }
