...
if 'especies_config_modebar' not in st.session_state:
    st.session_state.especies_config_modebar = {
        ...
        "modeBarButtonsToRemove": [
            "zoom2d", "pan2d", "lasso2d", "autoScale2d", "resetScale2d",
            "hoverClosestCartesian", "hoverCompareCartesian", "zoomIn2d", "zoomOut2d"
        ],
        "displaylogo": False,
    }
...
@st.cache_data(ttl=CACHE_TTL, max_entries=15, show_spinner="🔄 Processando dados de espécies...")
def get_species_table_ultra(obs_file, especies_file):
    ...
    try:
        ...
        last_month_species = obs_filtered[
            obs_filtered['observed_on'] < end_date
        ]['taxon_name'].nunique()
        ...
        species_stats = (
            ...
            .agg(['count', 'min', 'max'])
            .reset_index()
            .rename(columns={'count': 'count', 'min': 'first_observed', 'max': 'last_observed'})
            ...
            for obs_id in obs_ids:
                try:
                    response = session.get(
                        f"{API_PATH}/observations?id={obs_id}",
        ...
        for i in range(display_count):
            with cols[i]:
                try:
                    ...
                    taxon_name = str(row.get("taxon_name", "Unknown"))[:30]  # Truncate for performance
                    obs_id = row.get("id")
                    user_login = str(row.get("user_login", "Unknown"))[:15]  # Truncate for performance
...
for tab in st.tabs(
    ...
    with tab:
        ...
        with st.spinner(f"📊 Carregando dados de espécies {SPECIES_NAMES[i]}..."):
            try:
                table_species, last_month_species = get_species_table_ultra(obs_file, species_file)
            except Exception as e:
            ...
            try:
                if not table_species.empty:
                    ...
                    st.metric(
                        ...
                        species_count,
                        f"+{month_diff} último mês" if month_diff > 0 else f"{month_diff} último mês",
                    )
        ...
        with col3:
            ...
            with st.spinner("🔄 Carregando observações..."):
                try:
                    last_obs = get_obs_by_species_group_optimized(proj_id, GRUPOS_ESPECIES[i])
                except Exception as e:
            ...
            if len(last_obs) > 0:
                ...
                species_counts = last_obs_formatted['taxon_name'].value_counts().head(10)  # Limit for performance
                
                if not species_counts.empty:
                    summary_lines = [f"- {name}: {count}" for name, count in species_counts.items()]
                    st.markdown("\n".join(summary_lines))
                else:
