...
def get_last_species_from_obs(df_obs_path, df_photos_path):
    ...
    df_photos = pd.read_csv(df_photos_path, 
                           usecols=['id', 'photos_medium_url', 'attribution'])
    ...
    df_result = (
        species_obs
        .sort_values(by=["observed_on", "observed_on_time"])
    ...
    df_merged = pd.merge(
        df_result, df_unique_photos, on="id", how="inner"
    )
...
def prepare_species_display_data(df):
    ...
    df_display = df.copy()
    df_display["observed_on"] = pd.to_datetime(df_display["observed_on"]).dt.strftime("%d-%m-%Y")
    df_display["obs_url"] = df_display["id"].astype(str).apply(
        ...
        for i in range(max_display):
            ...
            with columns[i]:
                try:
                    ...
                    st.markdown(f":link: [MINKA](https://minka-sdg.org/observations/{obs_id})")
                    st.image(
                        photo_url,
...
with st.container():
    ...
    def process_last_observations():
        try:
            ...
            user_limited = (
                last_total
                .groupby('user_login', group_keys=False)
            ...
            with columns[col_idx]:
                try:
                    ...
                    st.image(
                        img_data['url'], 
                        use_container_width=True,
                        caption=img_data['taxon'][:30] + ('...' if len(img_data['taxon']) > 30 else '')
                    )
                except Exception as e:
...
def load_and_filter_species():
    ...
    try:
        ...
        df_species['first_date'] = pd.to_datetime(df_species['first_date'], errors='coerce')
...
with st.container():
    ...
    if not df_species_filtered.empty:
        st.success(f"Encontradas {len(df_species_filtered)} espécies novas nos últimos 30 dias")
