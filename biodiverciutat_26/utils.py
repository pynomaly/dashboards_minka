...
@st.cache_data(ttl=60)
def get_metrics_cities(projects, session=None):
...
def get_marine_terrestrial(df_obs):
    ...
    terrestrial_species["taxa_url"] = terrestrial_species.taxon_id.apply(
        lambda x: f"{config.HOME_PATH}/taxa/{int(x)}"
    )
...
def _get_identifiers(user_name, proj_id, session=None):
    if session is None:
        session = requests.Session()
    identifiers = f"{config.API_PATH}/observations/identifiers"
    params = {"project_id": proj_id, "user_login": user_name}
...
def get_participation_df(df_obs, proj_id):
    ...
    pt_users["identificacions"] = pt_users["participant"].apply(
        lambda x: _get_identifiers(x, proj_id, session=session)
    )
    pt_users["espècies"] = pt_users["participant"].apply(
        lambda x: _get_species(x, proj_id, session=session)
    )
    return pt_users
...
@st.cache_data(ttl=60)
def create_geo_df(main_project) -> pd.DataFrame:
    ...
    df_projects = pd.read_csv(
        f"{directory}/data/{main_project}_main_metrics_projects.csv"
    )
